...
type RuntimeTaskPayload = {
  ...
  status_transitions?: Array<{
    id: string;
    at: number;
    from_status?: RuntimeTaskPayload["status"] | null;
    to_status: RuntimeTaskPayload["status"];
    reason:
      | "task.create"
      | "task.transition"
      | "timeblock.pause"
...
function toTaskReplicationPayload(
...
export class FakeMeshRuntime {
  ...
  private readonly eventScopes = new Map<
    string,
    Map<string, RuntimeEventPayload>
  >();
  private readonly taskScopes = new Map<
    string,
    Map<string, RuntimeTaskPayload>
  >();
  private readonly completedTimeBlockScopes = new Map<
    string,
    Map<string, RuntimeTimeBlockPayload>
  >();
  ...
  private async handleRequest(
    ...
    if (url.pathname === "/eventlog" && req.method === "GET") {
      json(
        res,
        200,
        Array.from(this.getEventScope(scopeKey).values()).sort(
    ...
    if (url.pathname === "/eventlog/backup/sqlite" && req.method === "GET") {
      const payload = Buffer.from(
        JSON.stringify(Array.from(this.getEventScope(scopeKey).values())),
        "utf8",
      ).toString("base64");
      json(res, 200, {
        version: 1,
    ...
    if (url.pathname === "/eventlog/import/sqlite" && req.method === "POST") {
      ...
      const importedEvents = JSON.parse(
        Buffer.from(body.content_base64, "base64").toString("utf8"),
    ...
    if (url.pathname === "/tasks" && req.method === "GET") {
      json(
        res,
        200,
        Array.from(this.getTaskScope(scopeKey).values()).sort(
    ...
    if (url.pathname === "/tasks/backup/sqlite" && req.method === "GET") {
      const payload = Buffer.from(
        JSON.stringify(Array.from(this.getTaskScope(scopeKey).values())),
        "utf8",
      ).toString("base64");
      json(res, 200, {
        version: 1,
    ...
    if (url.pathname === "/tasks" && req.method === "POST") {
      const body = await this.readJsonBody<
        Partial<RuntimeTaskPayload> & { title: string }
      >(req);
      const now = Date.now();
      const task: RuntimeTaskPayload = {
        ...
        status_transitions: body.status_transitions ?? [
          {
            id: `${this.hostId}-task-init-${this.nextSequence}`,
            at: now,
            from_status: null,
            to_status: "pending",
            reason: "task.create",
    ...
    if (url.pathname === "/timeblocks" && req.method === "GET") {
      json(
        res,
        200,
        Array.from(this.getCompletedTimeBlockScope(scopeKey).values()).sort(
    ...
    if (
      url.pathname === "/timeblocks/replication/completed" &&
      req.method === "POST"
    ) {
      const body = await this.readJsonBody<{ block: RuntimeTimeBlockPayload }>(
        req,
      );
  ...
  private getCompletedTimeBlockScope(
    scopeKey: string,
  ): Map<string, RuntimeTimeBlockPayload> {
  ...
  private openSignalStream(req: IncomingMessage, res: ServerResponse): void {
    ...
    const lastSeenId = Array.isArray(lastEventId)
      ? lastEventId[0]
      : lastEventId;
