...
const TITLE_EDGE_INSET_PX =
  (HORIZONTAL_TRACK_HEIGHT_PX - TITLE_BUTTON_THICKNESS_PX) / 2;
...
const VERTICAL_TITLE_EDGE_INSET_PX =
  (VERTICAL_TRACK_WIDTH_PX - VERTICAL_TITLE_BUTTON_THICKNESS_PX) / 2;
...
const TIMELINE_LAYOUT_OPTIONS = [
  {
    ...
    label: "↕",
    title: "纵向模式",
    testId: "task-timeline-layout-vertical",
  },
  {
    key: "auto",
    label: "A",
    title: "自动模式",
    testId: "task-timeline-layout-auto",
  },
  {
    key: "horizontal",
    label: "⟷",
    title: "横向模式",
    testId: "task-timeline-layout-horizontal",
  },
] as const;

type TimelinePresetScale = Exclude<
  (typeof TIMELINE_SCALE_OPTIONS)[number]["id"],
  "custom"
>;
type TimelineScaleUnitBounds = Record<
...
function resolveTimelineScrollOffset(
  ...
  const positionRatio = clamp(
    (timestamp - displayTimeRange.start) / duration,
    0,
    1,
  );
  const maxScroll = Math.max(
    timelineMetrics.primaryCanvasSize - timelineMetrics.viewportPrimarySize,
    0,
  );
  const centeredOffset =
    positionRatio * timelineMetrics.primaryCanvasSize -
    timelineMetrics.viewportPrimarySize / 2;
...
function resolveTimelineIsHorizontal(
  layoutMode: TaskTimelineLayoutMode,
  isDesktop: boolean,
): boolean {
...
function resolveTimelineMetrics(
  ...
  const viewportPrimarySize = Math.max(
    ...
    isHorizontal
      ? HORIZONTAL_FALLBACK_VIEWPORT_PX
      : VERTICAL_FALLBACK_VIEWPORT_PX,
  );
  const pageCount = Math.max(totalDuration / scaleDuration, 1);

  return {
    primaryCanvasSize: Math.max(
      Math.ceil(viewportPrimarySize * pageCount),
      viewportPrimarySize,
    ),
    viewportPrimarySize,
    scaleDuration,
...
function resolveDisplayTimeRange(
  ...
  const paddedEnd =
    Math.ceil(Math.max(contentRange.end, scaleWindow.end) / step) * step;
  return {
...
function resolveHorizontalSegmentRounding(
  index: number,
  total: number,
): string {
  if (total <= 1) {
...
function resolveTimelineAxisUnit(
  range: TimelineRange,
): TimelineCustomScaleUnit {
...
function alignTimelineTickStart(
  timestamp: number,
...
function TimelineTaskTitleButton({
  ...
  const [collapsedPrimarySizePxState, setCollapsedPrimarySizePxState] =
    useState(collapsedPrimarySizePx);
  const [expandedPrimarySizePx, setExpandedPrimarySizePx] = useState(
    collapsedPrimarySizePx,
  );
  const verticalLines = useMemo(
    () => splitVerticalTitleLines(taskTitle),
    [taskTitle],
  );
  ...
  useEffect(() => {
    ...
    const measuredSize = isHorizontal
      ? label.scrollWidth + paddingPx
      : label.scrollHeight + paddingPx;
    setCollapsedPrimarySizePxState(
      Math.min(Math.max(measuredSize, 0), collapsedPrimarySizePx),
    );
    setExpandedPrimarySizePx(Math.max(measuredSize, 0));
  }, [collapsedPrimarySizePx, isHorizontal, taskTitle]);

  const primarySizePx = isHovered
    ? expandedPrimarySizePx
    : collapsedPrimarySizePxState;
  ...
  return (
    ...
    >
      <span
        ref={labelRef}
        className={`block overflow-hidden ${isHorizontal ? "truncate" : "text-center"}`}
      >
        {isHorizontal ? (
          taskTitle
        ) : (
    ...
    >
      {ticks.map((tick) => (
        <div
          ...
          style={
            isHorizontal
              ? { left: `${tick.position}%`, top: 4 }
              : { top: `${tick.position}%`, left: 4 }
          }
        >
          <span
            className={isHorizontal ? "-translate-x-1/2" : "-translate-y-1/2"}
          >
            {tick.label}
          </span>
        </div>
          ...
          {model.lanes.map((lane, laneIndex) => (
            ...
            >
              {lane.entries.map((entry) => {
                ...
                const entryStartPosition = toPercent(
                  entry.segments[0]?.startTime ??
                ...
                const titleMaxWidthPx = Math.max(
                  ((entryEndPosition - entryStartPosition) / 100) *
                    primaryCanvasSize -
                    TITLE_EDGE_INSET_PX * 2,
                  0,
                );
                  ...
                  <div key={entry.taskId}>
                    ...
                    {entry.segments.map((segment, index) => {
                      ...
                      return (
                        <button
                          ...
                          title={`${entry.taskTitle} · ${colors.label}`}
                          onClick={() => onSelectTask(entry.taskId)}
                          className={`absolute top-1/2 -translate-y-1/2 ${resolveHorizontalSegmentRounding(index, entry.segments.length)} ${isSelected ? "ring-2 ring-[#C75B3A]/40" : ""}`}
                          style={{
                            left: `${start}%`,
                            width: `${width}%`,
                    ...
                    {entry.terminalMarker ? (
                      <button
                        ...
                        title={`${entry.taskTitle} · ${STATUS_COLORS[entry.terminalMarker.status].label}`}
                        onClick={() => onSelectTask(entry.taskId)}
                        ...
                        style={{
                          ...
                          backgroundColor:
                            STATUS_COLORS[entry.terminalMarker.status].border,
                        }}
                      />
                    ) : null}
                  </div>
            ...
            >
              {lane.entries.map((entry) => {
                ...
                const titleMaxHeightPx = Math.max(
                  ((entryEndPosition - entryStartPosition) / 100) *
                    primaryCanvasSize -
                    VERTICAL_TITLE_EDGE_INSET_PX * 2,
                  0,
                );
                const isSelected = selectedTaskId === entry.taskId;
                    ...
                    {entry.segments.map((segment, index) => {
                      ...
                      return (
                        <button
                          ...
                          title={`${entry.taskTitle} · ${colors.label}`}
                          onClick={() => onSelectTask(entry.taskId)}
                          className={`absolute left-1/2 -translate-x-1/2 ${resolveVerticalSegmentRounding(index, entry.segments.length)} ${isSelected ? "ring-2 ring-[#C75B3A]/40" : ""}`}
                          style={{
                            top: `${start}%`,
                            height: `${height}%`,
...
function TimelineDetailPanel({
  ...
  return (
    <section
      ...
      className="border-t border-[#E7E5E4] bg-white px-5 py-4 dark:border-[#292524] dark:bg-[#1C1917]"
    >
      ...
      <div className="mt-3 space-y-2">
        {entry.segments.map((segment) => (
          <div
            ...
            className="flex items-center gap-2 text-xs text-[#57534E] dark:text-[#D6D3D1]"
          >
            <span
              className="h-2.5 w-2.5 rounded-full"
              style={{ backgroundColor: STATUS_COLORS[segment.status].bg }}
            />
...
export function TaskTimelinePage() {
  ...
  const initialCustomScaleDraft = useMemo(
    () => resolveCustomScaleDraftText(initialRange),
    [initialRange],
  );
  ...
  const [showPending, setShowPending] = useState(() =>
    readPersistedShowPending(),
  );
  const [layoutMode, setLayoutMode] = useState<TaskTimelineLayoutMode>(() =>
    readPersistedLayoutMode(),
  );
  const [isCustomScaleEditing, setIsCustomScaleEditing] = useState(false);
  const [customScaleDraft, setCustomScaleDraft] = useState(
    initialCustomScaleDraft,
  );
  const [timelineViewportSize, setTimelineViewportSize] = useState({
    width: 0,
    height: 0,
  });
  const isHorizontalTimeline = resolveTimelineIsHorizontal(
    layoutMode,
    isDesktop,
  );
  const scaleOptions = TIMELINE_SCALE_OPTIONS;
  const selectedScaleIndex =
    isCustomScaleEditing || typeof range === "object"
      ? scaleOptions.length - 1
      : Math.max(
          scaleOptions.findIndex((option) => option.id === range),
          0,
        );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const taskList = await taskService.listTasks(true);
  ...
  const model = useMemo(
    () => buildTaskTimelineModel(tasks, range, { showPending }),
    [range, showPending, tasks],
  );
  const scaleWindow = useMemo(
    () => resolveTimeRange(range, Date.now()),
    [range],
  );
  ...
  const selectedEntry =
    model.entries.find((entry) => entry.taskId === selectedTaskId) ?? null;
  ...
  useEffect(() => {
    ...
    const maxFocusStart = Math.max(
      displayTimeRange.end - timelineMetrics.scaleDuration,
    ...
    const nextScroll = clamp(
      focusRatio * timelineMetrics.primaryCanvasSize,
      0,
      maxScroll,
    );
    const nextFocusKey = [
  ...
  const handleJumpToNow = () => {
    ...
    const nextScroll = resolveTimelineScrollOffset(
      Date.now(),
      displayTimeRange,
      timelineMetrics,
    );
  ...
  return (
    ...
    >
      <div
        data-testid="task-timeline-page"
        ...
        <div className="border-b border-[#F0ECE8] px-5 py-3 dark:border-[#292524] md:px-8 lg:px-10">
          <div className="flex flex-col gap-3">
                        ...
                        <input
                          ...
                          onKeyDown={(event) => {
                            ...
                            if (event.key === "Escape") {
                              event.preventDefault();
                              setCustomScaleDraft(
                                resolveCustomScaleDraftText(range),
                              );
                              closeCustomScaleEditor();
                            }
                          }}
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-[#78716C] dark:text-[#A8A29E]">
              <span className="rounded-full bg-[#F5F0ED] px-2 py-1 dark:bg-[#292524]">
                比例尺：{formatRangeSummaryLabel(range)}
              </span>
              <span className="rounded-full bg-[#F5F0ED] px-2 py-1 dark:bg-[#292524]">
                任务：{model.entries.length}
              </span>
              <span className="rounded-full bg-[#F5F0ED] px-2 py-1 dark:bg-[#292524]">
                泳道：{model.lanes.length}
              </span>
            </div>
          </div>
        </div>
