...
function resolveEventTone(
  ...
  if (
    tags.has("block_start") ||
    tags.has("block_resume") ||
    tags.has("block_end")
  )
    return "success";
...
function buildTimeBlockEventLogQuery(
    ...
    return (
      ...
      >
        <div className="mx-auto max-w-7xl">
          <TaskBreadcrumb segments={[backLink]} current={{ label: "任务" }} />
        </div>
      </header>
    );
  }
...
function TimeBlockDetailShell({
  ...
  return (
    ...
    >
      <TimeBlockDetailHeader
        useDesktopPresentation={useDesktopPresentation}
        backLink={backLink}
      />
      <div
        className={`mx-auto ${useDesktopPresentation ? "max-w-7xl" : "max-w-4xl px-4 pt-4"}`}
      >
        {children}
      </div>
    </div>
...
function TimeBlockDetailLoadingState({
  ...
  return (
    <section
      data-testid="timeblock-detail-loading"
      aria-live="polite"
      className="space-y-4"
    >
      ...
      >
        <div className="space-y-4">
          {Array.from({ length: 2 }).map((_, index) => (
            ...
            >
              <div
                className="h-5 w-40 rounded-full bg-[#F5F0ED] dark:bg-[#292524]"
                aria-hidden="true"
              />
              <div
                className={`mt-4 grid gap-3 ${useDesktopPresentation ? "grid-cols-1 xl:grid-cols-3" : "sm:grid-cols-3"}`}
              >
                {Array.from({ length: 3 }).map((__, statIndex) => (
                  <div
                    key={`timeblock-loading-stat-${index}-${statIndex}`}
                    className="space-y-2"
                  >
                    <div
                      className="h-3 w-12 rounded-full bg-[#F5F0ED] dark:bg-[#292524]"
                      aria-hidden="true"
                    />
                    <div
                      className="h-4 w-full rounded-full bg-[#F5F0ED] dark:bg-[#292524]"
                      aria-hidden="true"
                    />
                  </div>
                ))}
              ...
              <div
                className="mt-4 h-16 rounded-xl bg-[#F5F0ED] dark:bg-[#292524]"
                aria-hidden="true"
              />
            </section>
          ))}
        ...
        <section className="rounded-2xl border border-[#E7E5E4] bg-white p-5 animate-pulse dark:border-[#292524] dark:bg-[#1C1917]">
          <div
            className="h-5 w-24 rounded-full bg-[#F5F0ED] dark:bg-[#292524]"
            aria-hidden="true"
          />
          <div className="mt-4 space-y-3">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={`timeblock-loading-log-${index}`}
                className="flex gap-3"
              >
                <div
                  className="mt-1 h-2 w-2 rounded-full bg-[#F5F0ED] dark:bg-[#292524]"
                  aria-hidden="true"
                />
                <div className="min-w-0 flex-1 space-y-2">
                  <div
                    className="h-4 w-2/3 rounded-full bg-[#F5F0ED] dark:bg-[#292524]"
                    aria-hidden="true"
                  />
                  <div
                    className="h-3 w-full rounded-full bg-[#F5F0ED] dark:bg-[#292524]"
                    aria-hidden="true"
                  />
                  <div
                    className="h-3 w-4/5 rounded-full bg-[#F5F0ED] dark:bg-[#292524]"
                    aria-hidden="true"
                  />
                </div>
              </div>
...
export function TimeBlockDetailPage() {
  ...
  const [desktopAdaptiveEnabled, setDesktopAdaptiveEnabled] = useState(() =>
    getDesktopAdaptiveEnabled(),
  );
  ...
  const backLink = isNowDomain
    ...
    : {
        label: taskDomainBackLink.sourceLabel,
        to: taskDomainBackLink.to,
        search: { main: "1" },
      };
  const returnTo = block
    ? isNowDomain
      ? `/eventlog/timeblocks/${block.startId}`
      : `/tasks/block/${block.startId}`
    : undefined;
  ...
  useEffect(() => {
    let disposed = false;
    const load = async () => {
      const trace = new PerfTrace("TimeBlockDetailPage loadDetail", {
      ...
      try {
        ...
        trace.step("load-block-snapshots", {
        ...
        let matchedBlock =
          blocks.find(
            (item) => item.id === blockId || item.startId === blockId,
          ) ?? null;
        if (!matchedBlock && activeBlock && activeBlock.startId === blockId) {
          matchedBlock = buildRunningTimeBlock(activeBlock, Date.now());
        }

        const taskIds = matchedBlock
          ? resolveTimeBlockRelatedTaskIds(matchedBlock)
          : [];
        const eventQuery = buildTimeBlockEventLogQuery(matchedBlock);
        trace.step("resolve-target-block", {
          found: Boolean(matchedBlock),
        ...
        const [tasks, events] = await Promise.all([
          Promise.all(taskIds.map((taskId) => taskService.getTask(taskId))),
          eventQuery
            ? getEventLogService().loadEvents(eventQuery)
            : Promise.resolve([] as Event[]),
        ]);
        trace.step("load-related-data", {
        ...
        trace.finish({
          eventCount: events.length,
          matchedBlockId: matchedBlock?.startId ?? null,
          outcome: "applied",
          taskCount: tasks.length,
        });
      } catch (error) {
  ...
  const timelineItems = useMemo(() => {
    ...
    return eventLogs
      .filter(
        (event) =>
          event.timestamp >= block.startTime &&
          event.timestamp <= block.endTime,
      )
    ...
    return (
      ...
      >
        <div className={useDesktopPresentation ? "py-2" : "py-1"}>
          <Link
            to={backLink.to}
            className="text-sm text-[#78716C] dark:text-[#A8A29E]"
          >
            ← 返回{backLink.label}
          </Link>
      ...
      >
        <div className="space-y-4">
          <section className="rounded-2xl border border-[#E7E5E4] bg-white p-5 dark:border-[#292524] dark:bg-[#1C1917]">
            <h1 className="text-lg font-semibold text-[#1C1917] dark:text-[#FAFAF9]">
              {view.summary.title}
            </h1>
            <div
              className={`mt-3 grid gap-3 text-sm text-[#57534E] dark:text-[#D6D3D1] ${useDesktopPresentation ? "grid-cols-1 xl:grid-cols-3" : "sm:grid-cols-3"}`}
            >
              <div>
                ...
                view.linkedTasks.map((task) => (
                  ...
                  >
                    <div className="relative flex items-center justify-between gap-3">
                      ...
                      >
                        <p className="truncate text-sm text-[#1C1917] dark:text-[#FAFAF9]">
                          {task.title}
                        </p>
