> 最后更新：`2026-04-16` | 更新者：`Codex` | 更新内容概要：`建立 references 索引、加载顺序与跨域分流规则。`
...
| `eventlog.md` | eventlog 读写、watch、按时间窗排查、备份导入、清空 | `eventlog` `watch` `since_id` `since_timestamp` `DELETE /eventlog` |
| `tasks.md` | 任务列表、创建、更新、状态迁移、取消、备份导入、复制 | `tasks` `transition` `cancel` `shortcut=true` `replication` |
| `timeblocks.md` | 时间块生命周期、start/stop/end、pause/resume、反馈、导入导出、复制 | `timeblocks` `stop` `end` `feedback` `taskStatusOutcomes` |
