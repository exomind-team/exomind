...
export interface TaskDagKeyboardOptions {
  ...
  onConnectStateChange: (
    state: { sourceId: string; type: "hard" | "soft" } | null,
  ) => void;
  onConnectExecute: (
    sourceId: string,
    targetId: string,
...
export function useTaskDagKeyboard(options: TaskDagKeyboardOptions): void {
  ...
  const applyPan = useCallback(
    (direction: Direction, deltaMs: number) => {
      if (!flowInstance) {
        return;
      }
  ...
  const applyZoom = useCallback(
    (zoomOut: boolean, deltaMs: number) => {
      if (!flowInstance) {
        return;
      }
      ...
      const normalizedZoom = Math.min(
        TASK_DAG_MAX_ZOOM,
        Math.max(TASK_DAG_MIN_ZOOM, nextZoom),
      );
      const container = getCanvasShell();
      if (!container) {
        flowInstance.setViewport({
  ...
  const applyContinuousAction = useCallback(
    (key: string, deltaMs: number) => {
  ...
  const traceDiscreteAction = useCallback(
    (action: string, fields: Record<string, unknown>, effect: () => void) => {
      const trace = new PerfTrace("TaskDagKeyboard discreteAction", {
        action,
        mode,
        immersive,
        selectedTaskId: selectedTaskId ?? null,
        focusedSeriesAnchorTaskId: focusedSeriesAnchorTaskId ?? null,
        connectSourceId: connectState?.sourceId ?? null,
        connectType: connectState?.type ?? null,
        ...fields,
      });
    ...
    [connectState, focusedSeriesAnchorTaskId, immersive, mode, selectedTaskId],
  );
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      const clearSelectionOrFocus = (triggerKey: string) => {
        if (selectedTaskId) {
          traceDiscreteAction(
            "clear-selected-node",
            { key: triggerKey },
      ...
      if (key === "Escape") {
        if (immersive) {
          traceDiscreteAction("exit-immersive", { key }, () => {
        ...
        if (mode === "connect" && connectState) {
          traceDiscreteAction("clear-connect-state", { key }, () => {
            onConnectStateChange(null);
            event.preventDefault();
          });
          return;
        }
        clearSelectionOrFocus(key);
        return;
      }
      ...
      if (key === "E" || key === "e") {
        if (!focusNodeId) {
          const nearestNodeId = findNearestNodeToViewportCenter(
            flowInstance,
            flowNodes,
          );
          if (nearestNodeId) {
            traceDiscreteAction(
              "select-viewport-center-node",
              {
                key,
                nextNodeId: nearestNodeId,
              },
              () => {
      ...
      if (
        ...
        const nextIndex =
          (currentIndex + delta + MODE_ORDER.length) % MODE_ORDER.length;
        traceDiscreteAction(
          "switch-mode",
          {
            key,
            nextMode: MODE_ORDER[nextIndex],
          },
          () => {
            event.preventDefault();
      ...
      if ((key === "Enter" || key === " ") && mode === "connect") {
        ...
        if (
          connectState &&
          focusNodeId &&
          focusNodeId !== connectState.sourceId
        ) {
          traceDiscreteAction(
            "connect-execute",
            {
              ...
              dependencyType: connectState.type,
            },
            () => {
              event.preventDefault();
              onConnectExecute(
      ...
      if (key === "Tab" && mode === "connect") {
        ...
        traceDiscreteAction(
          "quick-create",
          {
            key,
            quickCreateAnchorId,
            direction: event.shiftKey ? "upstream" : "downstream",
          },
          () => {
            event.preventDefault();
            if (event.shiftKey) {
