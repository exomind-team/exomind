...
> **状态**：已被 `docs/plans/2026-04-03-batch-q-task-dag-plan.md` 取代
> **分支**：`dev`
> **关联 Issue**：#501, #694, #700, #698, #639, #701, #660, #653
> **旧执行顺序**：#700 → #698 → #701 → #639 → #660 → #653
> **当前执行顺序**：Q-A `#694 #700 #698` → Q-B `#639 #701` → Q-C `#660 #653`
> **排除**：#591（已转向产品架构讨论，等 Hailaylin 参与设计后再定）
> **备注**：本文件仅保留早期讨论痕迹；后续实现、验收、issue 同步请以 2026-04-03 新计划为准。`PouchDB` 不在批次 Q 范围内；`ghost node` 术语统一替换为“次要节点”；`#639` 使用单一统一手动快照，不按横/竖屏、桌面/移动拆分。
...
| 标签过滤 | 选择"UI"标签 | 无该标签的节点隐藏或降为次要节点 | #653 |
| 取消标签 | 清空过滤 | 全部恢复 | #653 |
