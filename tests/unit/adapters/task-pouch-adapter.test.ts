...
import type { TaskStatusTransition } from "@/lib/types/task";

function sortTransitions(
  transitions: TaskStatusTransition[] | undefined,
...
describe("TaskPouchAdapter (ITaskPort)", () => {
  ...
  const testUserId = () =>
    `pouch-adapter-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  let userId: string;
  ...
  it("transitionTask changes status via state machine", async () => {
    ...
    expect(inProgress!.statusTransitions).toEqual(
    ...
    const persistedInProgress = await adapter.getTaskById(task.id);
    expect(persistedInProgress?.statusTransitions).toEqual(inProgress!.statusTransitions);

    const completed = await adapter.transitionTask(task.id, "completed");
    expect(completed!.status).toBe("completed");
    expect(completed!.completedAt).toBeDefined();
    expect(completed!.statusTransitions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
    ...
    const persistedCompleted = await adapter.getTaskById(task.id);
    expect(persistedCompleted?.statusTransitions).toEqual(
      completed!.statusTransitions,
    );

    const reloadedAdapter = new TaskPouchAdapter(userId);
    const roundTripped = await reloadedAdapter.getTaskById(task.id);
    expect(roundTripped?.statusTransitions).toEqual(completed!.statusTransitions);
  });
  ...
  it("transitionTask throws on invalid transition", async () => {
    ...
    await expect(
      adapter.transitionTask(task.id, "completed"),
    ).rejects.toThrow();
  });
  ...
  it("cancelTask sets status to cancelled with completedAt", async () => {
    ...
    expect(
      sortTransitions((await adapter.getTaskById(task.id))?.statusTransitions),
    ).toEqual(sortTransitions(cancelled!.statusTransitions));

    const reloadedAdapter = new TaskPouchAdapter(userId);
    expect(
      sortTransitions((await reloadedAdapter.getTaskById(task.id))?.statusTransitions),
    ).toEqual(
      sortTransitions(cancelled!.statusTransitions),
    );
  });
