...
## [2026-03-13] Ralph Loop - #511 语音输入归一化基础层立项与工作流收敛

### 执行摘要
- **任务**：为语音输入统一数据结构、统一链路、统一上下文表达建立独立跟踪项，并收敛后续执行工作流
- **结果**：完成 issue 拆分、独立 worktree 建立、协作约定固化
- **主要变更**：
  - 新建 GitHub issue `#511`，专门跟踪“统一语音输入归一化入口、上下文元数据与 EventLog/Signal 链路”
  - 明确 `#511` 作为 `#326 / #480` 的 foundation（基础前置），不再继续扩散子 issue
  - 建立独立开发分支 `feature/issue-511-voice-input-normalization`
  - 固化协作偏好：详细讨论写 issue，聊天窗口只保留简短任务摘要

### 遇到的问题

| 问题 | 原因 | 解决方案 | 优化建议 |
|------|------|----------|----------|
| 语音链路需求混入多个 issue | `#326` 已混入润色、分类、旧 topic 设想 | 新开 `#511` 只跟踪基础层 | 基础设施 issue 与上层产品/智能能力 issue 分离 |
| `dev` 正在被用户持续使用 | 直接改 `dev` 会影响日常使用 | 改为独立 worktree + 独立分支开发 | 中大型功能默认走 `1 issue + 1 worktree + 1 PR` |
| Windows worktree 下 `npx tsc --noEmit` 未命中本地 TypeScript | `npx` 在当前环境解析到非项目内 `tsc` | 改用 `bunx tsc --noEmit` 完成类型检查 | 在 Windows + Bun 场景统一优先使用 `bunx tsc --noEmit` |

### 有价值发现

1. **用户沟通偏好需要项目内显式记忆**
   - 聊天窗口输出应控制在单窗可读上限附近
   - 详细过程和设计讨论应沉淀到对应 issue
   - 这样更适合“人类看窗口、GitHub 做长期协作界面”的实际使用方式

2. **语音输入基础层必须单独立项**
   - 当前最值得先做的不是 Polish / Classifier / 指令模式
   - 而是统一 schema、统一 normalized topic、统一 metadata 持久化、统一目标上下文
   - 否则后续所有增强都会建立在不一致入口之上

3. **worktree 是保护 `dev` 可用性的必要手段**
   - 对“用户正在实际使用的开发主线”尤其重要
   - 可以把流程改成：主线使用与功能开发彻底隔离，再通过 PR 汇合
       └── SPEC-XXX.md
   ```
