...
describe('new focus timer widget wiring issue-175（新专注计时组件接线）', () => {
  ...
  it('uses NewFocusTimerWidget for new-mobile variant（新移动端改用新组件）', () => {
    expect(source).toContain("import { NewFocusTimerWidget");
    expect(source).toContain('<NewFocusTimerWidget ref={newFocusTimerWidgetRef} />');
  });
