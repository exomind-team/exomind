...
async fn create_and_delete_codex_runtime_agent_via_http_routes() {
    ...
    let fake_codex_command = env!("CARGO_BIN_EXE_fake-codex-app-server");
    unsafe {
        std::env::set_var("EXOMIND_CODEX_COMMAND", fake_codex_command);
    }

    let signal_pool = Arc::new(SignalPool::new(None));
    let host_id = "test-codex-agent-lifecycle".to_string();
    let state = AppState {
        port: 3920,
        host_id: host_id.clone(),
        registry: AgentRegistry::new(),
    ...
    let create_response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"kind":"codex","id":"codex-manual","name":"Manual Codex"}"#,
                ))
                .unwrap(),
        )
        .await
    ...
    let _create_response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .uri("/agents")
                .header("content-type", "application/json")
                .body(Body::from(r#"{"kind":"codex","id":"codex-stream"}"#))
                .unwrap(),
        )
        .await
        .unwrap();
