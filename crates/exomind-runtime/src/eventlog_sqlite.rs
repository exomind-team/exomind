...
impl SqliteEventLogStore {
    ...
    pub fn list_events(&self, user_key: &str) -> Result<Vec<EventRecord>, String> {
        ...
        let rows = statement
            .query_map(params![user_key], map_event_row)
    ...
    pub fn clear_events(&self, user_key: &str) -> Result<(), String> {
        let connection = self.connection();
        connection
            .execute("DELETE FROM eventlog_events WHERE user_key = ?1", params![user_key])
    ...
    pub fn replace_all(&self, user_key: &str, events: &[EventRecord]) -> Result<(), String> {
        ...
        tx.execute("DELETE FROM eventlog_events WHERE user_key = ?1", params![user_key])
    ...
    pub fn snapshot_bytes(&self) -> Result<Vec<u8>, String> {
        std::fs::read(&self.path).map_err(|error| format!("failed to read sqlite snapshot: {error}"))
    }
