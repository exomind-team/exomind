...
describe("task dag preferences（任务 DAG 偏好）", () => {
  ...
  it("reads runtime-backed dag preferences before local mirror（优先读取 Runtime 中的 DAG 偏好）", async () => {
    ...
    cacheModule.__primeRuntimeConfigForTests({
      ...
      "exomind:dag-interval-collapse": JSON.stringify({
        intervals: [{ startId: "task-a", endId: "task-c", collapsed: true }],
      }),
    ...
    expect(module.getTaskDagIntervalCollapseState()).toEqual({
      terminals: {
        "task-c": [{ startId: "task-a", collapsed: true }],
      },
    });
    expect(module.getTaskDagVisibility()).toEqual({
      collapsedUpstreamOf: ["task-a"],
    ...
    expect(module.getTaskDagViewport("LR")).toEqual({
      x: 12,
      y: 34,
      zoom: 0.8,
    });
  ...
  it("writes dag preferences through runtime-preferred storage（DAG 偏好通过 Runtime 优先存储写入）", async () => {
    ...
    expect(module.TASK_DAG_FOCUS_MODE_STORAGE_KEY).toBe(
      "exomind:dag-focus-mode",
    );
    expect(module.TASK_DAG_FOCUS_MODE_CHANGED_EVENT).toBe(
      "exomind:dag-focus-mode-changed",
    );
    expect(module.TASK_DAG_CONTROLS_STATE_STORAGE_KEY).toBe(
      "exomind:dag-controls-state",
    );
    expect(module.TASK_DAG_CONTROLS_STATE_CHANGED_EVENT).toBe(
    ...
    expect(
      JSON.parse(storage[module.TASK_DAG_NODE_SIZING_STORAGE_KEY] ?? "{}"),
    ).toEqual({
      fixedWidth: true,
      fixedHeight: true,
    });
    ...
    expect(storage[module.TASK_DAG_SEARCH_DRAFT_STORAGE_KEY]).toBe(
      "task keyword",
    );

    expect(
      module.setTaskDagSearchOptions({
        includeDescription: true,
        fuzzy: false,
        filterMode: true,
      }),
    ...
    expect(
      JSON.parse(storage[module.TASK_DAG_SEARCH_OPTIONS_STORAGE_KEY] ?? "{}"),
    ).toEqual({
      includeDescription: true,
      fuzzy: false,
      filterMode: true,
    ...
    expect(
      JSON.parse(storage[module.TASK_DAG_TAG_FILTER_STORAGE_KEY] ?? "{}"),
    ).toEqual({
      selectedTags: ["backend", "dag"],
      matchMode: "or",
    });
    ...
    expect(
      module.setTaskDagFocusedSeriesAnchorIds([
        "task-b",
        "task-x",
        "task-b",
        "",
    ...
    expect(
      JSON.parse(storage[module.TASK_DAG_FOCUSED_SERIES_STORAGE_KEY] ?? "[]"),
    ).toEqual(["task-b", "task-x"]);

    expect(
      module.setTaskDagIntervalCollapseState({
        terminals: {
          "task-c": [
            { startId: "task-a", collapsed: true },
            { startId: "task-a", collapsed: false },
          ],
    ...
    expect(
      JSON.parse(
        storage[module.TASK_DAG_INTERVAL_COLLAPSE_STORAGE_KEY] ?? "{}",
      ),
    ).toEqual({
      terminals: {
        "task-c": [{ startId: "task-a", collapsed: true }],
    ...
    expect(
      JSON.parse(storage[module.TASK_DAG_VISIBILITY_STORAGE_KEY] ?? "{}"),
    ).toEqual({
      collapsedUpstreamOf: ["task-x"],
      collapsedDownstreamOf: ["task-y"],
    });
    ...
    expect(
      module.setTaskDagControlsState({
        desktopViewOpen: false,
        desktopToolsOpen: true,
        desktopImmersivePanelOpen: "maybe",
        mobileViewOpen: true,
        mobileToolsOpen: "yes",
        tagSectionOpen: true,
      } as never),
    ...
    expect(
      JSON.parse(storage[module.TASK_DAG_CONTROLS_STATE_STORAGE_KEY] ?? "{}"),
    ).toEqual({
      desktopViewOpen: false,
      desktopToolsOpen: true,
      desktopImmersivePanelOpen: false,
    ...
    expect(
      JSON.parse(storage[module.TASK_DAG_VIEWPORT_STORAGE_KEY] ?? "{}"),
    ).toEqual({
      surface: "desktop",
      direction: "TB",
      x: 5,
  ...
  it("isolates dag viewport by surface and ignores legacy viewport payloads without surface（DAG 视口会按桌面/移动端隔离，并忽略旧版 payload）", async () => {
    ...
    expect(module.getTaskDagViewport("TB", "desktop")).toBeNull();
    expect(module.getTaskDagViewport("TB", "mobile")).toEqual({
      x: 10,
      y: 20,
      zoom: 0.5,
    });
  ...
  it("prefers grouped interval payloads over legacy mirrors when reading mixed storage（读取混合区间状态时以新模型为准）", async () => {
    storage["exomind:dag-interval-collapse"] = JSON.stringify({
      intervals: [
