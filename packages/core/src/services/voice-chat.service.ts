 ...
 *
 * 迁移自: src/lib/services/voice-chat.service.ts
 * 迁移时间: 2026-02-10
 */
...
export class VoiceChatService implements IVoiceChatService {
  ...
  private async checkAvailability(): Promise<void> {
    ...
    if (httpAvailable) {
      ...
      console.log('[VoiceChatService] 使用 WebSocket Adapter (浏览器受限，可能失败)');
  ...
  async startRecording(): Promise<void> {
    ...
    if (!this.isAvailable) {
      throw new Error('语音识别不可用，请配置 ASR 适配器');
    }
    ...
    if (this.adapterType === 'http') {
      ...
      // 标记录制状态
      (window as unknown as { __asrRecordingActive?: boolean }).__asrRecordingActive = true;

      // 调用 Promise 版本的 transcribe
      adapter
        .transcribe({
          lang: 'zh-CN',
          stream: this.stream,
        })
        .then((result: ASRResult) => {
  ...
  async stopRecording(): Promise<ASRResult | null> {
    ...
    // 停止录制状态
    (window as unknown as { __asrRecordingActive?: boolean }).__asrRecordingActive = false;
