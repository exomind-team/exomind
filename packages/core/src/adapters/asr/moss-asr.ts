 ...
 * - 返回带时间戳和说话人标识
 *
 * 迁移自: src/lib/adapters/asr/moss-asr.ts
 * 迁移时间: 2026-02-10
 */
...
export class MossASRAdapter implements IASRPort {
  ...
  constructor(config?: Partial<MOSSASRConfig>) {
    // 环境变量作为默认值，外部传入可覆盖
    // 注意：import.meta.env 在 NodeNext 模块下不可用
    // 使用 process.env 或外部配置
    const envApiKey = typeof process !== 'undefined' && process.env?.VITE_MOSS_API_KEY || '';
    this.config = {
      ...DEFAULT_CONFIG,
      apiKey: envApiKey,
      ...config,
   ...
   * 解析 MOSS API 返回结果
   */
  private parseMOSSResult(mossResponse: Record<string, unknown>): ASRResult {
    const transcription = mossResponse.asr_transcription_result as Record<string, unknown> | undefined;
    if (!transcription) {
      return {
        text: '',
        confidence: 0,
    ...
    let fullText = (transcription.full_text as string) || '';
    const segments = (transcription.segments as Array<Record<string, unknown>>) || [];

    // 如果 full_text 为空，从 segments 拼接
    if (!fullText && segments.length > 0) {
      fullText = segments.map((s) => (s.text as string) || '').join(' ');
    }
    ...
    if (segments.length > 0) {
      ...
      duration = (parseFloat(lastSegment.end_s as string) || 0) * 1000;
    }
  ...
  private async recordAudio(stream: MediaStream, _stopRecording?: () => void): Promise<Uint8Array> {
    return new Promise((resolve) => {
      ...
      const stopRecording = async () => {
        ...
        console.log(
          `[ASR-MOSS] WAV 编码完成: ${wavData.length} bytes (${targetSampleRate}Hz, ${(wavData.length / 1024).toFixed(2)}KB)`
        );
        resolve(wavData);
      };
      ...
      const startListening = () => {
        ...
        const checkStop = setInterval(() => {
          if (!(window as unknown as { __asrRecordingActive?: boolean }).__asrRecordingActive) {
