...
vi.mock("@xyflow/react", () => ({
  ReactFlow: ({
    ...
    onNodeContextMenu?: (
      _event: { preventDefault: () => void; clientX: number; clientY: number },
      node: { id: string; data?: Record<string, unknown> },
    ) => void;
    nodeTypes?: Record<
      string,
    ...
    return (
      <div data-testid="mock-react-flow">
        ...
        <button
          type="button"
          data-testid="mock-react-flow-pane-double"
          ...
          return (
            <button
              ...
              onContextMenu={(event) => {
                event.preventDefault();
                onNodeContextMenu?.(
                  { preventDefault: () => {}, clientX: 32, clientY: 48 },
                  node,
                );
              }}
            >
  ...
  Background: ({ variant }: { variant?: string }) => (
    <div
      data-testid="mock-react-flow-background"
...
async function openDesktopToolsPanel(): Promise<void> {
  if (!screen.queryByTestId("task-dag-tools-panel")) {
...
async function openDesktopImmersivePanel(): Promise<void> {
  if (!screen.queryByTestId("task-dag-search-panel")) {
    fireEvent.click(
      screen.getByTestId("task-dag-desktop-immersive-panel-toggle"),
    );
  }
...
async function openTagSection(): Promise<void> {
  ...
  await waitFor(() => {
    expect(
      screen.getByTestId("task-dag-tag-filter-mode-and"),
    ).toBeInTheDocument();
  });
}
...
async function openFocusSection(): Promise<void> {
  ...
  await waitFor(() => {
    expect(
      screen.getByTestId("task-dag-focus-mode-toggle"),
    ).toBeInTheDocument();
  });
}
...
describe("TaskDagPage issue-394（任务 DAG Wave 1 / Wave 2 / Wave 3）", () => {
  ...
  beforeEach(async () => {
    ...
    flowApiMocks.screenToFlowPosition.mockImplementation(
  ...
  it("renders full-canvas workspace with floating controls and three enabled modes", async () => {
    ...
    expect(
      screen.queryByTestId("task-dag-current-root-summary"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("task-dag-current-root-badge-task-a"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("task-dag-selected-panel"),
    ).not.toBeInTheDocument();
    expect(screen.getByTestId("task-dag-node-task-a").className).toContain(
      "border-[#16A34A]/60",
    );
    expect(screen.getByTestId("task-dag-node-slot-task-a").className).toContain(
      "h-40",
    );
    expect(screen.getByTestId("task-dag-node-slot-task-a").className).toContain(
      "w-40",
    ...
    expect(
      screen.getByTestId("task-dag-node-task-a").className.split(/\s+/),
    ).not.toContain("h-40");
    expect(
      screen.getByTestId("task-dag-node-task-a").className.split(/\s+/),
    ).not.toContain("w-40");
    ...
    expect(screen.getByTestId("task-dag-jump-to-root")).toHaveTextContent(
      "聚焦可执行",
    );
  ...
  it("keeps ReactFlow auto-fit disabled so execute-mode state changes do not re-fit the viewport", async () => {
    ...
    await waitFor(() => {
      expect(startBlockForTaskMock).toHaveBeenCalledWith("task-a", {
        mode: "countdown",
        minutes: 25,
      });
    });
  ...
  it("recomputes layout when hide-terminal removes visible nodes after execute updates", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("mock-react-flow-node-task-child"),
      ).toBeInTheDocument();
      expect(
        screen.getByTestId("mock-react-flow-node-task-side"),
      ).toBeInTheDocument();
    });

    const beforeFilteredUpdate = (
      flowApiMocks.lastProps as {
        nodes: Array<{ id: string; position: { x: number; y: number } }>;
      }
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByTestId("mock-react-flow-node-task-root"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByTestId("mock-react-flow-node-task-child"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByTestId("mock-react-flow-node-task-grandchild"),
      ).not.toBeInTheDocument();
    });

    const afterFilteredUpdate = (
      flowApiMocks.lastProps as {
        nodes: Array<{ id: string; position: { x: number; y: number } }>;
      }
    ).nodes.reduce<Record<string, { x: number; y: number }>>(
    ...
    expect(Object.keys(afterFilteredUpdate).length).toBeLessThan(
      Object.keys(beforeFilteredUpdate).length,
    );
  });
  ...
  it("does not auto-fit after nodes finish loading when no saved viewport exists", async () => {
    ...
    listTasksMock.mockImplementationOnce(
      () =>
  ...
  it("reacts to late storage-backed mode updates after mount（挂载后会响应晚到的 DAG 模式同步）", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("task-dag-mode-execute").className).toContain(
        "font-semibold",
      );
    });
  ...
  it("keeps late runtime dag mode instead of overwriting it on first mount（晚到 Runtime 模式不应被首帧旧值覆盖）", async () => {
    ...
    expect(screen.getByTestId("task-dag-mode-browse").className).toContain(
      "font-semibold",
    );
  ...
  it("switches dag direction, persists selection, and re-fits the viewport", async () => {
    ...
    await waitFor(() => {
      ...
      expect(flowApiMocks.fitView.mock.calls.length).toBeGreaterThan(
        fitViewCallCountBeforeChange,
      );
    });
  ...
  it("highlights the selected node instead of showing the old side panel", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("task-dag-node-task-b").className).toContain(
        "ring-[#C75B3A]/35",
      );
    });
    expect(
  ...
  it("clears the selected node with Escape when no higher-priority mode state is active", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("task-dag-detail-panel-desktop"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  it("opens the desktop detail panel on node click and shows dependency details", async () => {
    ...
    const detailPanel = await screen.findByTestId(
      "task-dag-detail-panel-desktop",
    );
    expect(detailPanel).toBeInTheDocument();
    expect(
      within(detailPanel).getByText("接入任务列表引导"),
    ).toBeInTheDocument();
    expect(
      within(detailPanel).getByText(
        "该任务目前仍被前置任务阻塞，需先完成对应依赖后才能启动。",
      ),
    ).toBeInTheDocument();
    expect(
      within(detailPanel).getByTestId("task-dag-detail-upstream-list"),
    ).toHaveTextContent("梳理 DAG 基础层");
    expect(
      within(detailPanel).getByText("当前节点没有后继依赖。"),
    ).toBeInTheDocument();
    expect(within(detailPanel).getByText("任务描述")).toBeInTheDocument();
    expect(
      within(detailPanel).getByText((content) =>
        content.includes("需要在浏览模式展示"),
      ),
    ).toBeInTheDocument();
  ...
  it("closes the detail panel on pane click and close button", async () => {
    ...
    fireEvent.click(screen.getByTestId("mock-react-flow-node-task-a"));
    expect(
      await screen.findByTestId("task-dag-detail-panel-desktop"),
    ).toBeInTheDocument();
  ...
  it("renders the mobile drawer variant when the viewport is not desktop", async () => {
    ...
    expect(
      await screen.findByTestId("task-dag-detail-panel-mobile"),
    ).toBeInTheDocument();
  });
  ...
  it("navigates to task detail with dag source on node double click and panel action", async () => {
    ...
    fireEvent.click(screen.getByTestId("mock-react-flow-node-task-b"));
    expect(
      await screen.findByTestId("task-dag-detail-open-task"),
    ).toBeInTheDocument();
  ...
  it("shows smart-mode terminal nodes as secondary nodes when they remain connected to unfinished work", async () => {
    ...
    expect(screen.getByTestId("task-dag-node-task-c").className).toContain(
      "opacity-35",
    );
  });
  ...
  it("restores hide-terminal and immersive preferences from localStorage on first render", async () => {
    ...
    expect(
      screen.queryByTestId("mock-react-flow-controls"),
    ).not.toBeInTheDocument();
  });
  ...
  it("highlights search matches and fades unmatched nodes", async () => {
    ...
    fireEvent.change(screen.getByTestId("task-dag-search-input"), {
      target: { value: "引导" },
    });
    await waitFor(() => {
      expect(
        screen.getByTestId("task-dag-search-match-count"),
      ).toHaveTextContent("1");
    });
  ...
  it("supports description, fuzzy, and filter search options with localStorage persistence", async () => {
    ...
    expect(
      JSON.parse(
        window.localStorage.getItem("exomind:dag-search-options") ?? "{}",
      ),
    ).toMatchObject({
    ...
    expect(window.localStorage.getItem("exomind:dag-search-draft")).toBe(
      "Markdown",
    );
    ...
    await waitFor(() => {
      expect(screen.getByTestId("task-dag-search-input")).toHaveValue(
        "Markdown",
      );
      expect(
        screen.getByTestId("task-dag-search-option-description"),
      ).toBeInTheDocument();
      expect(
        screen.getByTestId("task-dag-search-option-filter"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("treats tag selection as unified DAG search criteria and only hard-hides when filter mode is enabled", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId("task-dag-node-task-c").className,
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId("task-dag-node-task-b").className,
      ).not.toContain("opacity-35");
      expect(
        screen.getByTestId("task-dag-node-task-c").className,
      ).not.toContain("opacity-35");
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByTestId("mock-react-flow-node-task-d"),
      ).not.toBeInTheDocument();
    });

    expect(
      JSON.parse(window.localStorage.getItem("exomind:dag-tag-filter") ?? "{}"),
    ).toEqual({
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("task-dag-tag-filter-mode-or")).toHaveClass(
        "bg-[#FFF7ED]",
      );
    });
  });
  ...
  it("combines description search and tag search before unified filter-mode hiding", async () => {
    listTasksMock.mockResolvedValue([
      makeTask({
        id: "task-a",
        title: "前端节点",
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId("task-dag-node-task-a").className,
      ).not.toContain("opacity-35");
      expect(screen.getByTestId("task-dag-node-task-b").className).toContain(
        "opacity-35",
      );
  ...
  it("clears selection and shows a hidden-running notice only when unified filter mode hides the current running task", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByTestId("task-dag-hidden-running-filter-notice"),
      ).not.toBeInTheDocument();
      expect(
        screen.getByTestId("mock-react-flow-node-task-a"),
      ).toBeInTheDocument();
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId("task-dag-hidden-running-filter-notice"),
      ).toHaveTextContent("1");
      expect(
        screen.getByTestId("task-dag-tag-filter-clear"),
      ).toBeInTheDocument();
    });
  ...
  it("cycles terminal filtering through smart, strict, and show modes", async () => {
    ...
    expect(toggle).toHaveTextContent("弱化已结束");
    ...
    expect(window.localStorage.getItem("exomind:dag-hide-terminal")).toBe(
      "hide",
    );
    expect(toggle).toHaveTextContent("隐藏已结束");
    ...
    expect(toggle).toHaveTextContent("展示已结束");
  ...
  it("hides non-series nodes after switching from weak focus to strong focus", async () => {
    listTasksMock.mockResolvedValue([
    ...
    await waitFor(() => {
      expect(screen.getByTestId("task-dag-node-task-x").className).toContain(
        "opacity-35",
      );
    });
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("mock-react-flow-node-task-x"),
      ).not.toBeInTheDocument();
      expect(
  ...
  it("unions multiple focused series and shows anchor badges instead of a single anchor label", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId("mock-react-flow-node-task-x"),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(screen.getByTestId("task-dag-node-task-z").className).toContain(
        "opacity-35",
      );
      expect(
        screen.getByTestId("task-dag-node-task-a").className,
      ...
      expect(
        screen.getByTestId("task-dag-node-task-x").className,
      ).not.toContain("opacity-35");
      expect(
        screen.getByTestId("task-dag-node-task-y").className,
      ).not.toContain("opacity-35");
    });

    expect(
      screen.getByTestId("task-dag-focus-anchor-badge-task-b"),
    ).toHaveTextContent("聚焦锚点");
    expect(
      screen.getByTestId("task-dag-focus-anchor-badge-task-x"),
    ).toHaveTextContent("聚焦锚点");
    expect(screen.queryByText("当前锚点：B")).not.toBeInTheDocument();
    expect(
      JSON.parse(
        window.localStorage.getItem("exomind:dag-focused-series") ?? "[]",
      ),
    ).toEqual(["task-b", "task-x"]);
  ...
  it("allows collapsing the focus section even when focused anchors already exist", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("task-dag-focus-mode-toggle"),
      ).not.toBeInTheDocument();
    });
    expect(screen.getByText("已聚焦 1 个锚点")).toBeInTheDocument();
  });
  ...
  it("fully collapses the tag section and keeps only the and/or selection summary", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("task-dag-tag-filter-mode-and"),
      ).not.toBeInTheDocument();
    });
  ...
  it("switches background mode and persists the variant", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("mock-react-flow-background")).toHaveAttribute(
        "data-variant",
        "dots",
      );
    });
    ...
    expect(window.localStorage.getItem("exomind:dag-background-mode")).toBe(
      "lines",
    );

    fireEvent.click(screen.getByTestId("task-dag-background-none"));
    expect(
      screen.queryByTestId("mock-react-flow-background"),
    ).not.toBeInTheDocument();
    expect(window.localStorage.getItem("exomind:dag-background-mode")).toBe(
      "none",
    );
  ...
  it("shows fixed width and fixed height toggles in layout tools and persists them", async () => {
    ...
    expect(
      screen.getByTestId("task-dag-node-sizing-fixed-width"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("task-dag-node-sizing-fixed-height"),
    ).toBeInTheDocument();

    fireEvent.click(screen.getByTestId("task-dag-node-sizing-fixed-width"));
    expect(
      JSON.parse(
        window.localStorage.getItem(TASK_DAG_NODE_SIZING_STORAGE_KEY) ?? "{}",
      ),
    ).toEqual({
      fixedWidth: true,
      fixedHeight: false,
  ...
  it("hides low-priority badges for long titles in the base state and restores them when selected", async () => {
    listTasksMock.mockResolvedValue([
      makeTask({
        id: "task-long",
        title:
          "这是一个用于验证任务依赖图节点在长标题情况下仍优先展示名称而弱语义标签先退场的超长标题样例节点",
        createdAt: 10,
        updatedAt: 10,
      }),
    ]);
    window.localStorage.setItem(
      "exomind:dag-focused-series",
      JSON.stringify(["task-long"]),
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("mock-react-flow-node-task-long"),
      ).toBeInTheDocument();
    });
    ...
    expect(
      screen.queryByTestId("task-dag-focus-anchor-badge-task-long"),
    ).not.toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("task-dag-focus-anchor-badge-task-long"),
      ).toHaveTextContent("聚焦锚点");
    });
    ...
    expect(
      screen.getByTestId("task-dag-node-slot-task-long").className,
    ).toContain("h-40");
    expect(
      screen.getByTestId("task-dag-node-slot-task-long").className,
    ).toContain("w-40");
  ...
  it("applies the four auto-layout node sizing combinations to the content card", async () => {
    ...
    let cardClasses = screen
      .getByTestId("task-dag-node-task-a")
      .className.split(/\s+/);
    expect(cardClasses).toContain("max-w-40");
    expect(cardClasses).toContain("max-h-40");
    expect(cardClasses).not.toContain("w-40");
  ...
  it("switches modes and persists the latest mode to localStorage", async () => {
    ...
    expect(screen.getByTestId("task-dag-mode-connect")).toHaveTextContent(
      "编辑",
    );
  ...
  it("persists manual layout mode and restores dragged node positions after remount and mode toggles", async () => {
    ...
    const getNodeCardClassList = (taskId: string) =>
      screen.getByTestId(`task-dag-node-${taskId}`).className.split(/\s+/);

    expect(screen.getByTestId("task-dag-layout-mode-manual")).toHaveClass(
      "bg-[#FFF7ED]",
    );
    expect(
      (flowApiMocks.lastProps as { nodesDraggable?: boolean }).nodesDraggable,
    ).toBe(true);
    ...
    expect(
      JSON.parse(
        window.localStorage.getItem(TASK_DAG_MANUAL_LAYOUT_STORAGE_KEY) ?? "{}",
      ),
    ).toMatchObject({
      manualPositions: {
        "task-a": { x: 640, y: 320 },
    ...
    expect(window.localStorage.getItem(TASK_DAG_LAYOUT_MODE_STORAGE_KEY)).toBe(
      "auto",
    );
    expect(
      (flowApiMocks.lastProps as { nodesDraggable?: boolean }).nodesDraggable,
    ...
    await waitFor(() => {
      expect(getNodePosition("task-a")).toEqual(
        autoPositionBeforeReturningToManual,
      );
    });
    expect(getNodeDraggable("task-a")).toBe(true);
    expect(getNodeCardClassList("task-a")).toContain("nopan");
  ...
  it("emits focus-hard drag session logs during touch drags in manual layout", async () => {
    ...
    const interactionCalls = warnSpy.mock.calls
      ...
      .map((call) => ({
        message: call[1],
        payload: call[2] as Record<string, unknown> | undefined,
      }));

    expect(
      interactionCalls.some(
        (call) => call.message === "focus-hard:drag-session-start",
      ),
    ).toBe(true);
    expect(
      interactionCalls.some(
        (call) => call.message === "focus-hard:drag-session-end",
      ),
    ).toBe(true);

    const startCall = interactionCalls.find(
      (call) => call.message === "focus-hard:drag-session-start",
    );
  ...
  it("summarizes controlled vs instance flow node dimensions for hidden-node debugging", () => {
    const summary = summarizeTaskDagFlowNodeDimensions(
      [{ id: "task-a" }, { id: "task-b", width: 160, height: 160 }],
      [
        {
          id: "task-a",
  ...
  it("keeps manual node positions stable across direction changes and restores filtered nodes from the captured baseline", async () => {
    ...
    const manualSnapshotAfterEnter = JSON.parse(
      window.localStorage.getItem(TASK_DAG_MANUAL_LAYOUT_STORAGE_KEY) ?? "{}",
    );

    fireEvent.click(screen.getByTestId("task-dag-direction-tb"));
    expect(getNodePosition("task-a")).toEqual(
      filteredTaskAPosition ?? autoLrTaskA,
    );
    fireEvent.change(screen.getByTestId("task-dag-search-input"), {
      target: { value: "" },
    });
    ...
    expect(getNodePosition("task-b")).toEqual(
      manualSnapshotAfterEnter.manualBaselinePositions?.["task-b"] ??
        autoLrTaskB,
    );
  });
  ...
  it("shows sync only in manual layout and syncs only visible nodes to current auto-layout positions", async () => {
    ...
    expect(
      screen.queryByTestId("task-dag-layout-sync"),
    ).not.toBeInTheDocument();
    ...
    const syncedSnapshot = JSON.parse(
      window.localStorage.getItem(TASK_DAG_MANUAL_LAYOUT_STORAGE_KEY) ?? "{}",
    );

    expect(syncedSnapshot.manualPositions?.["task-c"]).toEqual({
      x: 777,
      y: 555,
    });
  ...
  it("renders dynamic key hints and hides them in immersive mode", async () => {
    ...
    expect(screen.getByTestId("task-dag-key-hints").className).toContain(
      "max-w-[50%]",
    );
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("task-dag-key-hints"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  it("collapses control panels into mobile toggles on narrow screens", async () => {
    ...
    expect(
      screen.queryByTestId("task-dag-search-panel"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("task-dag-tools-panel"),
    ).not.toBeInTheDocument();
  ...
  it("keeps view controls open by default on desktop while layout tools require explicit expansion and persist", async () => {
    const view = render(<TaskDagPage />);
  ...
  it("keeps mobile immersive search and tools panels visible after opening them", async () => {
    ...
    fireEvent.click(screen.getByTestId("task-dag-mobile-search-toggle"));
    expect(screen.getByTestId("task-dag-search-panel").className).not.toContain(
      "opacity-0",
    );

    fireEvent.click(screen.getByTestId("task-dag-mobile-tools-toggle"));
    expect(screen.getByTestId("task-dag-tools-panel").className).not.toContain(
      "opacity-0",
    );
    expect(screen.getByTestId("task-dag-legend-panel").className).not.toContain(
      "opacity-0",
    );
  });
  ...
  it("uses a pinned desktop immersive panel that stays visible until explicitly hidden", async () => {
    ...
    expect(
      screen.getByTestId("task-dag-desktop-immersive-panel-toggle").className,
    ).toContain("opacity-0");
    expect(
      screen.queryByTestId("task-dag-search-panel"),
    ).not.toBeInTheDocument();
  ...
  it("supports connect mode dependency toggle rules and surfaces cycle rejection", async () => {
    ...
    addDependencyMock.mockRejectedValueOnce(
      new Error("Adding dependency task-a → task-b would create a cycle"),
    );
  ...
  it("hides the browse-mode control panel while the detail panel is open", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("task-dag-hide-terminal-toggle"),
      ).toBeInTheDocument();
    });
    ...
    expect(
      screen.queryByTestId("task-dag-hide-terminal-toggle"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("task-dag-legend-hard-chip"),
    ).not.toBeInTheDocument();
    ...
    await waitFor(() => {
      const browseNode = (
        flowApiMocks.lastProps as {
          nodes: Array<{
            id: string;
            data: { isSelected?: boolean; showConnectHandles?: boolean };
          }>;
        }
  ...
  it("opens quick create from pane gestures in connect mode and submits a new task", async () => {
    ...
    expect(
      screen.queryByTestId("task-quick-create-dialog"),
    ).not.toBeInTheDocument();
    ...
    expect(
      await screen.findByTestId("task-quick-create-dialog"),
    ).toBeInTheDocument();
    fireEvent.change(screen.getByTestId("task-quick-create-title"), {
      target: { value: "连接模式新增任务" },
    });
    fireEvent.change(screen.getByTestId("task-quick-create-description"), {
      target: { value: "由测试创建" },
    });
    ...
    expect(
      await screen.findByTestId("task-dag-pane-context-menu"),
    ).toBeInTheDocument();
  ...
  it("creates dependency-aware quick tasks from pane click in connect mode, including upstream reversal", async () => {
    createTaskMock
      .mockResolvedValueOnce(
        makeTask({ id: "task-new-downstream", title: "连接模式下游任务" }),
      )
      .mockResolvedValueOnce(
        makeTask({ id: "task-new-upstream", title: "连接模式上游任务" }),
      );
  ...
  it("opens quick create when dragging from a source handle to blank space and creates a downstream hard dependency", async () => {
    createTaskMock.mockResolvedValueOnce(
      makeTask({ id: "task-blank-downstream", title: "空白下游任务" }),
    );
  ...
  it("uses target-handle blank drop to create an upstream soft dependency and persists the dropped position in manual mode", async () => {
    ...
    await waitFor(() => {
      expect(
        window.localStorage.getItem(TASK_DAG_LAYOUT_MODE_STORAGE_KEY),
      ).toBe("manual");
      expect(
        (flowApiMocks.lastProps as { nodesDraggable?: boolean }).nodesDraggable,
      ).toBe(true);
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("mock-react-flow-node-task-blank-upstream"),
      ).toBeInTheDocument();
    });
  ...
  it("supports keyboard quick-create shortcuts and Ctrl+Enter submission in connect mode", async () => {
    createTaskMock
      ...
      .mockResolvedValueOnce(
        makeTask({ id: "task-keyboard-upstream", title: "键盘上游任务" }),
      );
    ...
    fireEvent.change(screen.getByTestId("task-quick-create-description"), {
      target: { value: "使用 Ctrl+Enter 提交" },
    });
    fireEvent.keyDown(screen.getByTestId("task-quick-create-description"), {
      key: "Enter",
      ctrlKey: true,
    });
  ...
  it("reuses the current connect state when keyboard quick-create adds a soft dependency", async () => {
    createTaskMock.mockResolvedValueOnce(
      makeTask({ id: "task-soft-downstream", title: "键盘软依赖下游任务" }),
    );
    ...
    fireEvent.change(screen.getByTestId("task-quick-create-title"), {
      target: { value: "键盘软依赖下游任务" },
    });
    fireEvent.click(screen.getByTestId("task-quick-create-submit"));

    await waitFor(() => {
      expect(addDependencyMock).toHaveBeenCalledWith(
        "task-soft-downstream",
        "task-a",
        "soft",
      );
    });
  ...
  it("ignores stale task reload results so execute-mode updates do not collapse the dag to one node", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("mock-react-flow-node-task-root"),
      ).toBeInTheDocument();
      expect(
        screen.getByTestId("mock-react-flow-node-task-child"),
      ).toBeInTheDocument();
      expect(
        screen.getByTestId("mock-react-flow-node-task-grandchild"),
      ).toBeInTheDocument();
    });
  ...
  it("keeps all execute-mode nodes visible and leaves the viewport untouched across suspend and restart cycles", async () => {
    ...
    await waitFor(() => {
      expect(startBlockForTaskMock).toHaveBeenCalledWith("task-root", {
        mode: "countup",
      });
    });
    ...
    fireEvent.click(screen.getByTestId("mock-react-flow-node-task-child"));
    expect(
      await screen.findByTestId("task-dag-disassociate-dialog"),
    ).toBeInTheDocument();
    ...
    expect(
      (flowApiMocks.lastProps as { nodes: Array<{ id: string }> }).nodes,
    ).toHaveLength(3);
    expect(flowApiMocks.fitView).not.toHaveBeenCalled();
    expect(flowApiMocks.setViewport).not.toHaveBeenCalled();
  ...
  it("opens the multi-task end dialog in execute mode and submits task outcomes", async () => {
    ...
    expect(
      await screen.findByTestId("task-dag-end-dialog"),
    ).toBeInTheDocument();
    fireEvent.change(screen.getByTestId("task-dag-end-dialog-feedback"), {
      target: { value: "总结反馈" },
    });
  ...
  it("opens a status dialog before removing one task from a multi-task active block", async () => {
    ...
    expect(
      screen.queryByTestId("task-dag-disassociate-status-continue"),
    ).not.toBeInTheDocument();
  ...
  it("hides unavailable upstream/downstream actions in the context menu when a node cannot be safely folded", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("task-dag-context-toggle-upstream"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByTestId("task-dag-context-toggle-downstream"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  it("toggles focus-series from the browse-mode context menu and restores it after remount", async () => {
    ...
    expect(screen.getByTestId("task-dag-node-task-y").className).toContain(
      "opacity-35",
    );
    expect(screen.getByTestId("task-dag-node-task-a").className).not.toContain(
      "opacity-35",
    );
    expect(screen.getByTestId("task-dag-node-task-b").className).not.toContain(
      "opacity-35",
    );
    expect(screen.getByTestId("task-dag-node-task-c").className).not.toContain(
      "opacity-35",
    );
  ...
  it("creates an interval collapse from two browse-mode context-menu endpoint selections and persists it", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId("task-dag-interval-start-badge-task-d"),
      ).toHaveTextContent("起点 B");
      expect(
        screen.getByTestId("task-dag-interval-count-badge-task-d"),
      ).toHaveTextContent("3 个节点");
    });
  ...
  it("computes focus-series from the unified-search visible graph without bringing hidden branches back", async () => {
    listTasksMock.mockResolvedValue([
      makeTask({
        id: "task-a",
        title: "Batch Q DAG 主线 A",
        tags: ["batch-q", "dag", "focus"],
        createdAt: 10,
        updatedAt: 10,
      }),
      makeTask({
        id: "task-b",
        title: "Batch Q DAG 主线 B",
        tags: ["batch-q", "dag", "focus"],
        createdAt: 20,
        updatedAt: 20,
        dependsOn: [{ taskId: "task-a", type: "hard" }],
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByTestId("mock-react-flow-node-task-z"),
      ).not.toBeInTheDocument();
    });
  ...
  it("allows multiple anchors after filtering splits one underlying chain into separate visible series", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("task-dag-focus-anchor-badge-task-a"),
      ).toBeInTheDocument();
      expect(
        screen.getByTestId("task-dag-focus-anchor-badge-task-c"),
      ).toBeInTheDocument();
      expect(screen.getByTestId("task-dag-node-task-x").className).toContain(
        "opacity-35",
      );
  ...
  it("removes anchors from the current focused series without affecting other focused series", async () => {
    ...
    expect(
      screen.queryByTestId("task-dag-focus-anchor-badge-task-b"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("task-dag-focus-anchor-badge-task-c"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByTestId("task-dag-focus-anchor-badge-task-x"),
    ).toBeInTheDocument();
