...
import { sendPtyWsTextInput, type PtyInputTarget } from "./pty-input";
...
export function PtyPromptComposer({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const response = await sendPtyWsTextInput(target, payload);
      if (!response.ok) {
  ...
  return (
    ...
    >
      <div
        className={compact ? "flex items-end gap-2" : "flex items-end gap-3"}
      >
        <Textarea
          ref={textareaRef}
          ...
          className={[
            ...
            compact
              ? "max-h-24 text-[11px] leading-5"
              : "max-h-32 text-xs leading-5",
          ].join(" ")}
        />
        <button
          type="button"
        ...
        >
          {isSending ? (
            ...
            <SendHorizontal size={compact ? 12 : 14} />
          )}
        </button>
      </div>
      {errorMessage ? (
        <p
          ...
          className={
            compact
              ? "mt-1 text-[10px] text-[#FCA5A5]"
              : "mt-2 text-[11px] text-[#FCA5A5]"
          }
        >
          {errorMessage}
        </p>
