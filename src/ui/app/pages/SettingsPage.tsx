...
export function SettingsPage() {
  ...
  const [nowWorkbenchOverlayEnabled, setNowWorkbenchOverlayEnabledState] = useState<boolean>(
    () => getNowWorkbenchOverlayEnabled()
  );
  ...
  useEffect(() => {
    return subscribeNowWorkbenchOverlayEnabledChanges((enabled) => {
      setNowWorkbenchOverlayEnabledState(enabled);
    });
  }, []);
  ...
  const handleNowWorkbenchOverlayEnabledToggle = (enabled: boolean) => {
    const normalizedValue = setNowWorkbenchOverlayEnabled(enabled);
    setNowWorkbenchOverlayEnabledState(normalizedValue);
  };
          ...
          <section ref={sectionNotificationRef} className="space-y-2" data-testid="new-settings-desktop-vc-section-notification">
            ...
            <SectionCard>
              ...
              <div data-testid="new-settings-input-section">
                ...
                <Divider />
                <div data-testid="new-settings-now-overlay-enabled-row">
                  <SettingRow
                    icon={<Monitor className="h-[18px] w-[18px] text-[#78716C]" />}
                    label="启用当下工作台悬浮窗"
                    right={(
                      <Switch
                        data-testid="new-settings-now-overlay-enabled-switch"
                        checked={nowWorkbenchOverlayEnabled}
                        onCheckedChange={handleNowWorkbenchOverlayEnabledToggle}
                      />
                    )}
                  />
                </div>
                <div className="pb-[14px] pl-[46px] pr-4">
                  <span className="text-xs text-[#A8A29E]">
                    开启后会在桌面最上层显示固定尺寸的当下工作台悬浮窗，关闭后完全隐藏。
                  </span>
                </div>
                {voiceShortcutAsrProvider === 'volcano' ? (
                  <>
