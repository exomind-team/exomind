 ...
 * └─────────────────────────────────────────┘
 *
 * 迁移自: src/components/Chat/ChatPage.tsx
 * 迁移时间: 2026-02-10
 */
...
import { EventStorage } from '@exomind/core';
import type { Event as StorageEvent } from '@exomind/core';
import { useSyncStore } from '../stores/sync-store';
import type { Event } from '@exomind/shared';
      ...
      <div className="flex-1 overflow-auto p-3 sm:p-6" data-testid="event-list">
        {events.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">开始计时或输入内容记录事件</p>
          </div>
        ) : (
            ...
            {Array.from(groupedEvents.entries()).map(([date, dateEvents]) => (
              <div key={date}>
                <div className="flex items-center justify-center mb-3 sm:mb-4">
                  <span className="text-xs text-muted-foreground bg-muted px-2 sm:px-3 py-1 rounded-full">{date}</span>
                </div>
                <div className="space-y-2 sm:space-y-3">
                  {dateEvents.map((event) => (
                    <div key={event.id} className="flex gap-2 sm:gap-3">
                      <Avatar className="h-6 w-6 sm:h-8 sm:w-8 shrink-0">
                        <AvatarFallback className={getEventBgColor(event)}>{getEventIcon(event)}</AvatarFallback>
                      </Avatar>
                      <div className="max-w-[75%] sm:max-w-[70%]">
                        <div className={`inline-block px-3 py-1.5 sm:px-4 sm:py-2 rounded-2xl ${getEventBgColor(event)}`}>
                          {getEventPrefix(event) && (
                            <span className="text-xs opacity-75 mr-1">{getEventPrefix(event)}</span>
                          )}
                          <p className="text-xs sm:text-sm break-words">{event.content}</p>
                        </div>
                        <p className="text-xs text-muted-foreground mt-0.5 sm:mt-1">{formatTime(event.timestamp)}</p>
                      </div>
                    </div>
                  ))}
                </div>
