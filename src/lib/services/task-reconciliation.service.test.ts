...
function createSummary(overrides: Partial<TaskReplicationSummary> = {}): TaskReplicationSummary {
  return {
    schemaVersion: 1,
    scopeKey: 'anonymous',
    taskCount: 1,
...
function createRuntimeTask(overrides: Partial<RuntimeTaskPayload> = {}): RuntimeTaskPayload {
  return {
    id: 'task-1',
...
function createPullResult(overrides: Partial<TaskReplicationPullResult> = {}): TaskReplicationPullResult {
  return {
    schemaVersion: 1,
    scopeKey: 'anonymous',
...
describe('TaskReconciliationService', () => {
  ...
  it('skips reconciliation when local and remote summaries already match', async () => {
    ...
    expect(taskBackupService.exportPeerTasksAsSqliteSnapshot).not.toHaveBeenCalled();
  ...
  it('fails closed on scope mismatch before attempting auto-repair', async () => {
    taskBackupService.getTaskReplicationSummary.mockResolvedValue(createSummary({
      scopeKey: 'profile-local',
      revisionHash: 'hash-local',
    }));
    taskBackupService.getPeerTaskReplicationSummary.mockResolvedValue(createSummary({
  ...
  it('pulls remote pages and converges without snapshot fallback when remote is newer', async () => {
    ...
    taskBackupService.getPeerTaskReplicationSummary.mockResolvedValue(remoteSummary);
    taskBackupService.pullPeerTaskReplicationBatch
      .mockResolvedValueOnce(createPullResult({
    ...
    expect(taskBackupService.pullPeerTaskReplicationBatch).toHaveBeenNthCalledWith(1, 'peer-3', undefined, 50);
    expect(taskBackupService.pullPeerTaskReplicationBatch).toHaveBeenNthCalledWith(2, 'peer-3', nextCursor, 50);
  ...
  it('falls back to sqlite snapshot when pull does not converge', async () => {
    ...
    const snapshotImport: TaskImportResult = { imported: 3, skipped: 1, total: 4 };
    ...
    taskBackupService.getTaskReplicationSummary
      ...
      .mockResolvedValueOnce(remoteSummary);
    taskBackupService.getPeerTaskReplicationSummary.mockResolvedValue(remoteSummary);
    taskBackupService.pullPeerTaskReplicationBatch.mockResolvedValue(createPullResult({
    ...
    taskBackupService.importTasksFromSqliteSnapshot.mockResolvedValue(snapshotImport);
    ...
    expect(taskBackupService.exportPeerTasksAsSqliteSnapshot).toHaveBeenCalledWith('peer-4');
    expect(taskBackupService.importTasksFromSqliteSnapshot).toHaveBeenCalledWith(
  ...
  it('attempts pull when revision hash differs at the same watermark', async () => {
    ...
    taskBackupService.getPeerTaskReplicationSummary.mockResolvedValue(remoteSummary);
    taskBackupService.pullPeerTaskReplicationBatch.mockResolvedValue(createPullResult({
      items: [createRuntimeTask({ id: 'task-same-watermark', updated_at: 500 })],
      hasMore: false,
      nextCursor: undefined,
      summary: remoteSummary,
    }));
  ...
  it('keeps unresolved drift when remote is newer but represents deletions we cannot auto-merge', async () => {
    ...
    expect(taskBackupService.exportPeerTasksAsSqliteSnapshot).toHaveBeenCalledWith('peer-delete');
  });
});
