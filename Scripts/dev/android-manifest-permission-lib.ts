...
function buildMainActivityWithMulticastLock(packageName: string, newline: string): string {
  return [
    ...
    '',
    '    // Push to WebView via global JS callback',
    '    runCatching {',
    '      val webView = window?.decorView?.findViewById<WebView>(',
    '        android.R.id.content',
    '      )?.findViewWithTag<WebView>("tauri_webview")',
    '        ?: findWebView(window?.decorView)',
    '',
    '      webView?.evaluateJavascript(',
    '        """',
    '        (function() {',
    '          if (window.__EXOMIND_KEYBOARD_STATE__) {',
    '            window.__EXOMIND_KEYBOARD_STATE__($hasKeyboard, "$keyboardType");',
    '          }',
    '          window.dispatchEvent(new CustomEvent("exomind:keyboard-state", {',
    '            detail: { hasHardwareKeyboard: $hasKeyboard, keyboardType: "$keyboardType" }',
    '          }));',
    '        })();',
    '        """.trimIndent(),',
    '        null',
