...
3. 以脚本状态机结果为准，执行当前唯一最高优先级动作。
   - 处理 `create-draft-pr` / `sync-pr-body` 时，优先使用：`npx tsx Scripts/dev/worker-agent/index.ts pr sync`
   - `next-action` / `restore` 输出里的 `context.targetLanguage` 是当前对外文本的语言真相
   - 之后所有新写的 PR 标题、PR 正文、issue/PR 评论、执行异议文本，都必须跟随这个 `targetLanguage`
   - 若调用 `render-comment` / `render-body` / `render-dissent-*`，传入 `--language <targetLanguage>`
4. 每轮只做一个动作，不要混做多件事。
5. 动作完成后：
   - 若本轮已经处理并吸收了当前反馈批次，先执行：`npx tsx Scripts/dev/worker-agent/index.ts cursor sync`
...
7. 若 `next-action` 返回 `wait-for-update`，则调用等待命令并阻塞，直到被审阅者 / 人类 / CI / 人测事件唤醒。
