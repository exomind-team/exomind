...
- 人工验收必须能在 UI 或 debug route 中看见 peer、interface、pairing operation/error、authorization、delivery/error 状态；点击操作后的显示仍以 backend snapshot/SSE 为准。
...
### Phase 4：Reticulum provider contract（signed queue、dynamic UDP/dynamic TCP、JSONL/file data-plane 与 mDNS bootstrap 已完成）

目标：用当前 ExoNet-Reticulum 公开 API 实现最小真实 provider，并把 fake gateway 证明过的功能搬到真实 Reticulum packet / 后续 link 上。当前已完成 queue-backed signed EventLog happy path、dynamic UDP 双 provider EventLog 纵切、dynamic TCP server/client 四域纵切、JSONL/file EventLog physical medium 纵切和 mDNS `ret_port` bootstrap projection；link lifecycle、observed sender metadata、ack/receipt 和更完整路径发现后置。
...
- EventLog 真实 provider happy path 已在 queue、dynamic UDP、dynamic TCP server/client、JSONL 与 file 上恢复；TCP server/client 真实 provider 已按 Phase 2 的四域验收扩展到 Task、TimeBlock active/completed 与 Proposal；mDNS bootstrap 已能把 Reticulum interface endpoint 投影为未授权 discovered peer。下一步补 local JSON/default startup config 等剩余日用入口，并做 mDNS bootstrap 真实局域网双进程验收。
