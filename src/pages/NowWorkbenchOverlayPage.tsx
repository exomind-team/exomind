...
const OVERLAY_POSITION_PERSIST_DEBOUNCE_MS = 250;
...
function NowWorkbenchOverlayRuntimePage({ props }: { props: NowWorkbenchOverlayPageProps }) {
  ...
  useSignalStream({
    ...
    enabled: controller.profileReady,
  });
  if (!controller.profileReady) {
    return renderRuntimeStarting({
      ready: false,
      detail: '正在等待主窗口同步隔离档案…',
    });
  }
...
function NowWorkbenchOverlayPageContent(props: NowWorkbenchOverlayPageContentProps) {
  ...
  useEffect(() => {
    ...
    const persistPendingPosition = () => {
      ...
      setNowWorkbenchOverlayPosition(position);
    };
    ...
    void getCurrentWindow().onMoved((event) => {
      ...
      persistTimer = window.setTimeout(
        persistPendingPosition,
        OVERLAY_POSITION_PERSIST_DEBOUNCE_MS,
      );
    }).then((cleanup) => {
      if (disposed) {
        cleanup();
