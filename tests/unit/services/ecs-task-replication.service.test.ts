...
describe("ecs-task-replication.service", () => {
  const sampleTask: TaskNode = {
    ...
    tags: [],
    statusTransitions: [
      buildInitialTaskStatusTransition("task-replication-1", 1_700_000_000_000),
    ],
  ...
  beforeEach(() => {
    mocks.getCurrentProfileOrLegacyIdMock
      .mockReset()
      .mockReturnValue("profile-local");
    mocks.applyReplicationSnapshotMock
      .mockReset()
  ...
  it("projects replicated task into local RT adapter（把远端任务复制快照投影进本地 RT）", async () => {
    ...
    await expect(projectTaskReplicationUpsert(payload)).resolves.toBe(
      "inserted",
    );
  ...
  it("normalizes runtime-shaped replicated task payload before projecting（运行时 snake_case 任务载荷应先正规化再投影）", async () => {
    const runtimeTaskPayload = {
      ...
      status_transitions: [
        {
          id: "task-runtime-shaped:task.create:1700000100000",
