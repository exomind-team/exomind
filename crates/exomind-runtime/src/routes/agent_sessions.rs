...
mod tests {
    ...
    use crate::agent::session::TOOL_GROUP_RECENT_EVENTS;
    ...
    async fn fake_openai_recent_events_group_handler(
        AxumState(call_count): AxumState<Arc<AtomicUsize>>,
        Json(payload): Json<Value>,
    ) -> Json<Value> {
        let turn = call_count.fetch_add(1, Ordering::SeqCst);
    ...
    async fn spawn_fake_openai_recent_events_group_server() -> (String, oneshot::Sender<()>) {
        let app = Router::new()
            .route("/chat/completions", post(fake_openai_recent_events_group_handler))
            .with_state(Arc::new(AtomicUsize::new(0)));
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr: SocketAddr = listener.local_addr().unwrap();
        let (shutdown_tx, shutdown_rx) = oneshot::channel::<()>();
    ...
    #[tokio::test]
    async fn route_expands_recent_events_tool_group_via_rust_session_layer() {
        let temp = tempfile::tempdir().unwrap();
        let state = test_state(temp.path());
        let (base_url, shutdown_tx) = spawn_fake_openai_recent_events_group_server().await;
        let app = router().with_state(state);
            ...
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        json!({
                            "systemPrompt": "你是测试助手",
                            "toolGroups": [TOOL_GROUP_RECENT_EVENTS],
    ...
    #[tokio::test]
    async fn route_rejects_combining_tools_and_tool_groups() {
        let temp = tempfile::tempdir().unwrap();
        let state = test_state(temp.path());
        let app = router().with_state(state);
    ...
    #[tokio::test]
    async fn route_rejects_missing_scope_key_for_runtime_tool_group() {
        let temp = tempfile::tempdir().unwrap();
        let state = test_state(temp.path());
        let app = router().with_state(state);
