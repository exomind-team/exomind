...
export function PtyTerminalPage({ ptyId }: { ptyId?: string }) {
  ...
  useEffect(() => {
    ...
    const verifyPty = async () => {
      ...
      try {
        const response = await fetch(`${rtBaseUrl}/pty`, {
  ...
  const recoverDisconnectedSession = async (): Promise<boolean> => {
    ...
    for (const step of recoverySteps) {
      const response = await fetch(
        `${rtBaseUrl}/sessions/${encodeURIComponent(matchingSession.id)}`,
        {
          method: "PATCH",
  ...
  const handleStop = async () => {
    ...
    try {
      const response = await fetch(
        `${rtBaseUrl}/pty/${encodeURIComponent(ptyId)}/stop`,
        {
          method: "POST",
          headers: buildHeaders(),
        },
  ...
  return (
    <div
      data-testid="pty-terminal-page-root"
      data-pty-id={ptyId}
    ...
    >
      ...
      <div className="flex-1 overflow-hidden">
        {isDisconnected ? (
          ...
          >
            <div className="border-b border-[#292524] px-6 py-4 text-center">
              ...
              <p className="mt-1 text-xs text-[#A8A29E]">
                对应 PTY 已不存在，RT
                可能已经重启。下方保留关闭前历史；如需结束，可点击上方“结束”将会话收敛为已完成。
              </p>
            </div>
            <div className="flex-1 overflow-hidden">
              <PtyTerminal
          ...
          <div
            data-testid="pty-terminal-page-live"
