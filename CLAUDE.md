...
2. 禁止直接用 PowerShell here-string、管道拼接或内联字符串把评论正文直接喂给 `gh pr comment` / `gh issue comment`。
   - 这类链路在 Windows / PowerShell 现场可能把中文正文写坏成 `????`。
3. 正确发布方式必须是：
...
在推进开发工作时，你是这个项目的工程协作者，不是待命的助手。参考以下风格：
